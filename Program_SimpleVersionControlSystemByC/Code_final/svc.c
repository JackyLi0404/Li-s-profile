...
//    new_commit->file_track->change_list = NULL;   // 释放change_list
    ...
    file_track_struct *ft_ptr = new_commit->file_track;
    if (ft_ptr != NULL) {
        // 深拷贝 checkout_struct 的 file track->file_list
...
//        printf("%commit_pre\n", byte);
    ...
    while (cl_ptr != NULL) {
        ...
        switch (cl_ptr->ptr->change_type) {
            ...
            case DELETION:
                id += 85973;
                // TODO 从file_list中删除文件
                file_track_delete_file(new_commit->file_track, cl_ptr->ptr->file_hash);
                break;
            case MODIFICATION:
                id += 9573681;
                break;
...
void print_commit(void *helper, char *commit_id) {
    ...
    message = NULL;
    change_list_struct *cl_ptr = commit->file_track->change_list;
    while (cl_ptr != NULL) {
        char *type = NULL;
        char *file_name = cl_ptr->ptr->file_name;
        int is_continue = -1;
        switch (cl_ptr->ptr->change_type) {
            case ADDITION:
                type = "+";
        ...
        if (cl_ptr->ptr->change_type == MODIFICATION) {
            printf("    [%d --> %d]\n", cl_ptr->ptr->file_hash_pre, cl_ptr->ptr->file_hash);
        } else {
            printf("\n");
        }
        cl_ptr = cl_ptr->next;
    ...
    // 统计非deletion的文件数量
    int active_file_num = 0;
    cl_ptr = commit->file_track->change_list;
    while (cl_ptr != NULL) {
        if (cl_ptr->ptr->change_type != DELETION) {
            active_file_num++;
        }
        cl_ptr = cl_ptr->next;
    }
...
//    printf("    Tracked files (%d):\n", commit->file_track->file_num);
    ...
    while (cl_ptr != NULL) {
        if (cl_ptr->ptr->change_type != DELETION) {
            printf("    [%10d] %s\n", cl_ptr->ptr->file_hash, cl_ptr->ptr->file_name);
        }
        cl_ptr = cl_ptr->next;
    }
}
...
int svc_branch(void *helper, char *branch_name) {
    ...
    free_file_track(new_co->file_track);
    new_co->file_track=deepcopy_file_track(h->head_checkout->file_track);
...
void file_track_delete_file(file_track_struct *ft, int file_hash) {
    if (file_list_delete_file(&(ft->file_list), file_hash) == 1) {
        ft->file_num--;
    }
}
...
// 删除成功返回1
// 删除失败返回-1
int file_list_delete_file(file_list_struct **fl_input, int file_hash) {
    file_list_struct *fl = *fl_input;

    file_list_struct *fl_head = construct_file_list(NULL);
    ...
    while (fl_mid != NULL) {
        ...
        if (fl_mid->ptr->file_hash == file_hash) {
            // 删除fl_mid
            fl_pre->next = fl_next;
            fl_mid->next = NULL;

            // 考虑情况：删除的是file_list的第一个节点
            if (fl_mid == fl) {
                fl = fl_head->next;
            }

            free_file_list_single(fl_mid);
            break;
        } else {
            fl_pre = fl_pre->next;
            fl_mid = fl_mid->next;
