    ...
    public static void paymentInterface(List<ProductMenu> productMenus, Transaction transaction){
        ...
        do{ transaction.list_items();
            ...
            switch (selection){
                ...
                case 1:
//                    pay by cash
                    CashPayMenu.pay(productMenus, transaction);
                    break;
...
//                    back to main menu
    ...
    public static void list_5_items(Transaction transaction){
        try {
            Connection c = sqliteFunction.connect();
            String query = "SELECT user_last_5_items FROM User WHERE user_name ='"+ transaction.getUsername() + "';";
